...
function lessonCard(lesson, small=false) { const [start,end,code,detail] = lesson, [name,icon,color] = info(code), display = settingsForChild().display; const time = display === "icons" ? "" : `<time>${start}${small ? "–" : "<br>– "}${end}</time>`; const iconHtml = display === "times" ? "" : `<span class="subject-icon" aria-hidden="true">${icon}</span>`; const timeCards = display === "icons" ? "" : `<div class="lesson-times" aria-label="Tid ${start} till ${end}"><span><small>Start</small><b>${start}</b></span><span><small>Slut</small><b>${end}</b></span></div>`; return small ? `<article class="week-lesson ${display}" style="--subject:${color}">${time}<strong>${name}</strong>${detail?`<small>${detail}</small>`:""}</article>` : `<article class="lesson ${display}" style="--subject:${color}"><div class="lesson-info"><div class="lesson-main">${iconHtml}<div><h3>${name}</h3>${detail?`<p>${detail}</p>`:""}</div></div>${timeCards}</div></article>`; }
