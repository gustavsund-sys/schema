...
function updateNextLesson() {
  ...
  if (actualDay < 0 || actualDay > 4) { container.innerHTML = `<article class="next-card finished"><p class="eyebrow">NÄSTA LEKTION</p><h2>Ingen skola idag</h2><p>Vi ses nästa skoldag.</p></article>`; return; }
  const now = new Date(), nowMinutes = now.getHours() * 60 + now.getMinutes();
  const upcoming = schedules[child][actualDay].find(lesson => lesson[2] !== "LUNCH" && minutesSinceMidnight(lesson[0]) > nowMinutes);
  if (!upcoming) { container.innerHTML = `<article class="next-card finished"><p class="eyebrow">NÄSTA LEKTION</p><h2>Dagens skoldag är slut</h2><p>Fint jobbat idag.</p></article>`; return; }
  const [name, icon] = info(upcoming[2]), remaining = minutesSinceMidnight(upcoming[0]) - nowMinutes;
  container.innerHTML = `<article class="next-card"><p class="eyebrow">NÄSTA LEKTION</p><h2>${icon} ${name} · ${upcoming[0]}</h2><p class="countdown">${name} börjar om ${remaining} min</p></article>`;
}
function lessonCard(lesson, small=false) { const [start,end,code,detail] = lesson, [name,icon,color] = info(code); return small ? `<article class="week-lesson" style="--subject:${color}"><time>${start}–${end}</time><strong>${name}</strong>${detail?`<small>${detail}</small>`:""}</article>` : `<article class="lesson" style="--subject:${color}"><time>${start}<br>– ${end}</time><div class="lesson-info"><div><h3>${name}</h3>${detail?`<p>${detail}</p>`:""}</div><span class="subject-icon" aria-hidden="true">${icon}</span></div></article>`; }
function render() {
...
applyTheme(theme);
$("#theme-toggle").onclick=()=>{theme=theme === "dark" ? "light" : "dark"; localStorage.setItem("mitt-skolschema-tema",theme); applyTheme(theme);};
const dialog=$("#import-dialog"); [$("#open-import"),$("#open-import-home")].forEach(b=>b.onclick=()=>dialog.showModal());
