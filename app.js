...
async function saveWeek(childId){try{await setDoc(doc(db,"skolschema",childId),{week:schedules[childId],updatedAt:serverTimestamp()},{merge:true});editorMessage("Ändringen är sparad!");}catch(error){console.error("Kunde inte spara schema",error);const code=error.code || "okänt fel";editorMessage(code === "permission-denied" ? "Firebase nekade ändringen. Kontrollera Firestore-regeln och att du är inloggad med rätt konto." : `Kunde inte spara (${code}). ${error.message || "Försök igen."}`,true);throw error;}}
