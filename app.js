...
applyTheme(theme);
async function saveAppearance(childId){const settings=childSettings[childId];await setDoc(doc(db,"skolschema",childId),{preferences:{accent:settings.accent,display:settings.display,theme:settings.theme||"light"}},{merge:true});}
function requireAdminForAppearance(){if(signedInUser?.uid===adminUid)return true;alert("Logga in som administratör för att ändra utseendet för alla enheter.");return false;}
$("#theme-toggle").onclick=async()=>{if(!child||!requireAdminForAppearance())return;theme=theme === "dark" ? "light" : "dark";localStorage.setItem("mitt-skolschema-tema",theme);settingsForChild().theme=theme;saveSettings();applyTheme(theme);try{await saveAppearance(child);}catch{alert("Kunde inte synka inställningen till Firebase.");}};
...
$("#accent-choices").onclick=async e=>{const button=e.target.closest("[data-accent]");if(!button||!requireAdminForAppearance())return;settingsForChild().accent=button.dataset.accent;saveSettings();applyAccent();refreshSettingsDialog();render();try{await saveAppearance(child);}catch{alert("Kunde inte synka inställningen till Firebase.");}};
$("#display-choices").onclick=async e=>{const button=e.target.closest("[data-display]");if(!button||!requireAdminForAppearance())return;settingsForChild().display=button.dataset.display;saveSettings();refreshSettingsDialog();render();try{await saveAppearance(child);}catch{alert("Kunde inte synka inställningen till Firebase.");}};
...
function syncChild(childId) { onSnapshot(doc(db,"skolschema",childId), snapshot => { const data=snapshot.data()||{}, week=fromFirestoreWeek(data.week), preferences=data.preferences;if(week)schedules[childId]=week;if(preferences&&typeof preferences==="object"){childSettings[childId]={...defaultSettings(),accent:preferences.accent||"purple",display:preferences.display||"both",theme:preferences.theme||"light"};saveSettings();if(child===childId){theme=childSettings[childId].theme;applyTheme(theme);}}if(child===childId)render(); }, () => {}); }
...
async function saveWeek(childId){try{await setDoc(doc(db,"skolschema",childId),{week:toFirestoreWeek(schedules[childId]),updatedAt:serverTimestamp()},{merge:true});editorMessage("Ändringen är sparad!");}catch(error){console.error("Kunde inte spara schema",error);const code=error.code || "okänt fel";editorMessage(code === "permission-denied" ? "Firebase nekade ändringen. Kontrollera Firestore-regeln och att du är inloggad med rätt konto." : `Kunde inte spara (${code}). ${error.message || "Försök igen."}`,true);throw error;}}
